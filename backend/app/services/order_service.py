...
async def create_order(data: OrderCreate, client_id: int, role: Role, db: AsyncSession) -> Order:
    ...
    order = Order(**data.model_dump(), client_id=client_id, status=Status.OPEN)
    try:
...
async def get_orders(db: AsyncSession, page: int, limit: int, category_id: Optional[int], status: Optional[Status]):
...
async def change_status(order_id: int, new_status: Status, db: AsyncSession) -> Order:
    order = await get_order(order_id, db)
